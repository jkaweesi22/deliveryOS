...
export class CodebergProviderStub implements DeliveryProvider {
  readonly name = 'codeberg' as const;

  private err(op: string): Error {
    return new Error(
      `Codeberg / Gitea provider is not implemented yet (${op}). See docs/PROVIDER_MATRIX.md.`
    );
  }
